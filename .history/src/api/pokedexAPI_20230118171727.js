...
export const obtenerTodo = async () => {
    ...
    try {
        let resultado2=[];
        let resultado = await axiosInstance.get('?limit=100000&offset=0');
      //  console.log({resultado})
        for(let i=0;i<=resultado.data.results.length-1 ;i++){
            const {data}=await axios.get(resultado.data.results[i].url)
            //  console.log(data)
              if(data.sprites.front_default!==null){
                  resultado2.push({url:resultado.data.results[i].url,name:resultado.data.results[i].name})
              }
        }
