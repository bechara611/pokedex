...
export const obtenerDetalle = async (nombre="charmander") => {

    try {
        const resultado = await axiosInstance.get(nombre);
...
export const obtenerImagenes = async (vector = [], hasta = 48, desde = 0) => {
    try {
        ...
        for (let i = 0; i < vectorImagenes.length; i++) {
            let { moves, abilities, name, sprites, weight, stats } = { ...vectorImagenes[i] }
            vectorReducido.push({
