...
import { obtenerDetalle, obtenerImagenes, obtenerTodo } from "../api/pokedexAPI";
import { setPokemonActivo, settotalActual, setvectorPokemones, setvectorPokemones2, setvectorPokemonesCompleto } from "../pokedex/slices/slicePokedex";
...
//Obtenemos del metodo de API los resultados de los pokemon para primero guardar cuantos son en total,
    ...
    const GetDetalle=async(nombre="charmander")=>{
        try {
             const {data} = await obtenerDetalle(nombre);
           //  console.log(resultado.data.results[0])
        
            
             console.log({resultado})
            dispatch(setPokemonActivo(...data))
    ...
    const getObtenerImagenes=async(vector=[],hasta=48,desde=0)=>{
        ...
        if(busqueda===true){ resultado = await obtenerImagenes(vector,hasta,desde)}
