...
const slicePokedex = createSlice({
  ...
  reducers: {
    ...
    setBusquedaPokemon:(state,action)=>{
      state.vectorPokemones= state.vectorPokemones.filter((pokemon)=>pokemon.name=='bulbasaur')
    }
    
  }
});

export const {setPokemonActivo,settotalActual,setvectorPokemones,setvectorPokemonesCompleto} = slicePokedex.actions
