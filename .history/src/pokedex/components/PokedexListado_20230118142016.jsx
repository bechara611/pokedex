...
export const PokedexListado = () => {
    ...
    const {paginaActual,resultadoPaginaActual} = useSelector((state)=>state.ui)
    ...
    useEffect(() => {
        const funcion=async()=>{
            let resultadito= await getObtenerImagenes(vectorPokemones,6)
         
        }
        if (vectorPokemones.length>1) {
            funcion();
        }  
    }, [vectorPokemones])
